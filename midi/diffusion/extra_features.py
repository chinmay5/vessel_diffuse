...
class ExtraFeatures:
    ...
    def __call__(self, z_t):
        ...
        if self.features_type == 'cycles':
            ...
            # Would return num_connected_component and the k highest eigen values for the graph
            ...
            # We return these values for computing the loss term
...
class NodeCycleFeaturesSingle:
    """
    Computes the number of cycles for a single graph only
    """
    def __init__(self):
        self.kcycles = KNodeCycles()

    def __call__(self, adj_matrix):
        # Remove the edge attr. We just want connectivity for the computation here
...
class EigenFeatureSingleGraph(EigenFeatures):
    """
    The class computes eigenvalues and eigenvectors for a single sample only
    """
    def __init__(self):
    ...
    def __call__(self, graph_data):
        """
        Returns the top-5 eigenvectors and eigenvalues.
        :param graph_data: pytroch geometric graph object
        :return:
        """
        # to_dense_adj adds the batch size of 1
        ...
        if self.mode == 'eigenvalues':
            ...
            # TODO: Think of ways to include the nonlcc indicator as well.
            # Maybe it helps or maybe it does not since it just tells the nodes that they should be reachable.
            # That does not handle the problem with equidistant nodes being viable candidates.
            # Retrieve eigenvectors features
            # nonlcc_indicator, k_lowest_eigenvector = get_eigenvectors_features(vectors=eigvectors,
            #                                                                    node_mask=mask,
            #                                                                    n_connected=n_connected_comp)
